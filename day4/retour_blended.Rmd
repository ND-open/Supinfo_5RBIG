...
```{r}
iris_num <- iris[,1:4]
groups <- kmeans(x = iris_num, centers = 3)

iris %>% 
        ggplot(., aes(x = Sepal.Length, y = Sepal.Width, colour = factor(groups$cluster) )) +
        geom_point()
```
...
factoextra::fviz_pca_ind(iris_pca, col.ind = iris$Species)
factoextra::fviz_pca_var(iris_pca)
factoextra::fviz_pca_biplot(iris_pca, col.ind = iris$Species)
```
